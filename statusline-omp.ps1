...
function Select-StatuslineFunctions {
    ...
    .DESCRIPTION
      ...
      which one statusline.ps1 actually runs), and no definition may exit. Any
...
function Get-StatuslineConfigCode {
    ...
    .DESCRIPTION
      Located by their opening text:
        Defaults  from `$HomeDir = [string]$HOME` to the $ConfigKeys loop, without
                  $ScriptDir and $ScriptPath, which describe statusline.ps1 and are
                  set by the caller;
    ...
    try {
        ...
        foreach ($statement in $Ast.EndBlock.Statements) {
            ...
            switch ($stage) {
                0 {
                    if ($text.StartsWith('$HomeDir = [string]$HOME', [System.StringComparison]::Ordinal)) { $stage = 1; [void]$defaultsCode.AppendLine($text); [void]$nodes.Add($statement) }
                    break
                }
                1 {
...
function Get-OmpState {
    ...
    .DESCRIPTION
      ...
      abort the chain where statusline.ps1 skips one statement):
        1. the config helpers, the defaults and the config-loading statements; then
           the wrapper's own files are appended to $ConfigVisitedPaths, so no state
           path (and no float target) may point at them;
        2. the state-layer functions and Format-Eta, each defined exactly once, and
           the gate statements, matched one by one against a fixed list;
    ...
    $ompStateNames = @('ConvertTo-Epoch', 'Try-BoundedDouble', 'Get-BoundedInt', 'ConvertTo-LocalFullPath', 'Test-LocalPathSyntax',
        ...
        'ConvertFrom-LimitName', 'Get-StateDirectorySnapshot', 'ConvertFrom-BurnRecord', 'Remove-BurnTemporaries',
    ...
    if ($Cfg -is [System.Collections.IDictionary] -and $Cfg.Contains('BURN_FILE') -and $Cfg.Contains('RL5H_FILE') -and $Cfg.Contains('RL7D_FILE') -and $Cfg.Contains('VL_LIMIT_SYNC')) { $ompGuard++ }
